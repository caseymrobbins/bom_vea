...
def plot_reconstructions(model, samples, split_idx, output_path, device='cuda'):
    ...
    with torch.no_grad():
        ...
        axs[1,0].set_ylabel('Full Recon', fontsize=10)
        axs[2,0].set_ylabel('Core Only', fontsize=10)
...
def plot_cross_reconstruction(model, samples, split_idx, output_path, device='cuda'):
    ...
    with torch.no_grad():
        ...
        fig.suptitle('Cross-Reconstruction: x1 STRUCTURE + x2 APPEARANCE', fontsize=14)
        for i in range(n):
        ...
        axs[0,0].set_ylabel('x1 (structure)', fontsize=10)
        axs[1,0].set_ylabel('x2 (appearance)', fontsize=10)
    ...
    with torch.no_grad():
        ...
        for prefix, offset, path, title in [
            ...
            for d in range(num_dims):
                ...
                axs[d,0].set_ylabel(f'dim {d}', fontsize=8)
...
def plot_training_history(histories, output_path):
    ...
    # v13: structure/appearance instead of texture
    axs[0,2].plot(histories['structure_loss'], label='structure')
    axs[0,2].plot(histories['appearance_loss'], label='appearance')
    axs[0,2].set_title('Swap Losses'); axs[0,2].legend(); axs[0,2].grid(True, alpha=0.3)
