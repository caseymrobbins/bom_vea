# bom_vae_v8_contrastive.py
# Contrastive texture transfer - detail dims must make output texture MORE similar to x2 than x1
...
OUTPUT_DIR = '/content/outputs_bom_v8'
EVAL_SAMPLES = 10000
...
def texture_distance(img1_features, img2_features):
    """Compute texture distance using gram matrices across VGG layers"""
    dist = 0.0
    for f1, f2 in zip(img1_features, img2_features):
        g1 = gram_matrix(f1)
        g2 = gram_matrix(f2)
        dist += (g1 - g2).pow(2).sum(dim=[1, 2])  # Per-sample distance
    ...
    def calibrate(self, epoch=0):
        ...
        for name in self.loss_names:
            if self.samples[name]:
                ...
                print(f"  {name:20s}: median={median:.4f} -> ref={self.references[name]:.4f} ({change:+.1f}%)")
            else:
                print(f"  {name:20s}: NO SAMPLES - keeping {self.references[name]:.4f}")
        print("="*60 + "\n")
...
RECALIBRATION_BATCHES = 100

# Loss names - texture_contrastive replaces texture_transfer
LOSS_NAMES = [
    'pixel', 'edge', 'perceptual',           # Recon group
    'core_mse', 'core_edge', 'cross', 'texture_contrastive',  # Core group
...
def compute_contrastive_texture_loss(r_sw, x1, x2):
    """
    Contrastive texture loss:
    r_sw = x1_core + x2_detail should have texture CLOSER to x2 than to x1
    
    Loss = max(0, dist(r_sw, x2) - dist(r_sw, x1) + margin)
    
    If r_sw texture is closer to x2: loss is 0 (good!)
    If r_sw texture is closer to x1: loss is positive (bad - detail didn't transfer texture)
    """
    # Get features for all three
    ...
    # Texture distances (per sample)
    dist_to_x2 = texture_distance(r_sw_feat, x2_feat)  # Should be small
    dist_to_x1 = texture_distance(r_sw_feat, x1_feat)  # Should be large
    
    # Contrastive margin loss
    # We want dist_to_x2 < dist_to_x1, so loss = relu(dist_to_x2 - dist_to_x1 + margin)
    margin = 0.1  # r_sw should be at least margin closer to x2 than x1
...
def grouped_bom_loss(recon, x, mu, logvar, z, model, refs):
    """Compute BOM loss with contrastive texture goal."""
    ...
    if B >= 4:
        ...
        # CONTRASTIVE TEXTURE: r_sw should be texturally closer to x2 than x1
        texture_loss, dist_to_x2, dist_to_x1 = compute_contrastive_texture_loss(r_sw, x1, x2)
        g_texture = refs.goal(texture_loss, 'texture_contrastive')
    else:
    ...
    tex_info = {
        'loss': texture_loss.item() if isinstance(texture_loss, torch.Tensor) else texture_loss,
    ...
    return (loss, groups, min_group_idx, group_values, individual_goals,
            ...
            pixel_mse.item(), edge_loss.item(), detail_ratio.item(), tex_info)
...
histories = {
    ...
    'texture_dist_x2': [], 'texture_dist_x1': [],  # Track both distances
...
print("\n" + "="*120)
print(f"BOM VAE v8 - CONTRASTIVE TEXTURE TRANSFER - {EPOCHS} EPOCHS")
print("NEW: Texture goal is CONTRASTIVE - r_sw must be texturally closer to x2 than x1")
print("     Loss = relu(dist(r_sw,x2) - dist(r_sw,x1) + margin)")
...
for epoch in range(1, EPOCHS + 1):
    ...
    for batch_idx, (x, _) in enumerate(pbar):
        ...
        loss, groups, min_idx, group_vals, ind_goals, kl, cov_pen, ssim_v, mse, edge, det_ratio, tex_info = result
        ...
        with torch.no_grad():
            ...
            epoch_data['texture_loss'].append(tex_info['loss'])
            epoch_data['texture_dist_x2'].append(tex_info['dist_x2'])
            epoch_data['texture_dist_x1'].append(tex_info['dist_x1'])
        ...
        # Show texture distances in progress bar
        pbar.set_postfix({
            'loss': f"{loss.item():.2f}", 
            'min': f"{g_np.min():.3f}",
            ...
            'd2': f"{tex_info['dist_x2']:.3f}",
            'd1': f"{tex_info['dist_x1']:.3f}",
        })
        ...
        if batch_idx % 400 == 0 and batch_idx > CALIBRATION_BATCHES:
            ...
            print(f"           Texture: dist_x2={tex_info['dist_x2']:.4f} dist_x1={tex_info['dist_x1']:.4f} (want x2<x1)")
    ...
    if all_mu_core:
        ...
        core_var_norm = core_var_norm + 1e-8
    ...
    d2 = histories['texture_dist_x2'][-1] if histories['texture_dist_x2'] else 0
    d1 = histories['texture_dist_x1'][-1] if histories['texture_dist_x1'] else 0
    ...
    print(f"\nEpoch {epoch:2d} | Loss: {histories['loss'][-1]:.3f} | Min: {histories['min_group'][-1]:.3f} | "
          f"SSIM: {histories['ssim'][-1]:.3f} | Tex: {tex_goal:.2f} | "
          f"d(x2)={d2:.3f} d(x1)={d1:.3f} | Det: {histories['detail_ratio'][-1]:.3f}")
...
ax.set_title('BOM VAE v8: Group Balance (Contrastive Texture)', fontsize=14)
...
# Individual goals with texture distances
...
axs[0,1].set_title('Core Group (+ Contrastive Texture)'); axs[0,1].legend(); axs[0,1].grid(True, alpha=0.3)
...
axs[1,0].plot(ep, histories['split'], label='split goal')
axs[1,0].plot(ep, histories['detail_ratio'], label='detail_ratio')
axs[1,0].axhline(0.1, color='r', ls='--', alpha=0.5)
axs[1,0].axhline(0.5, color='r', ls='--', alpha=0.5)
axs[1,0].set_title('Split Constraint'); axs[1,0].legend(); axs[1,0].grid(True, alpha=0.3)

# Texture distances - THE KEY PLOT
axs[1,1].plot(ep, histories['texture_dist_x2'], 'b-', lw=2, label='dist to x2 (want LOW)')
axs[1,1].plot(ep, histories['texture_dist_x1'], 'r-', lw=2, label='dist to x1 (want HIGH)')
axs[1,1].set_title('Texture Distances (want blue < red)'); axs[1,1].legend(); axs[1,1].grid(True, alpha=0.3)
axs[1,1].set_xlabel('Epoch')

axs[1,2].plot(ep, histories['core_active'], 'b-', lw=2, label='Core active')
axs[1,2].plot(ep, histories['detail_active'], 'r-', lw=2, label='Detail active')
axs[1,2].axhline(64, color='k', ls='--', alpha=0.3)
axs[1,2].set_title('Active Dimensions'); axs[1,2].legend(); axs[1,2].grid(True, alpha=0.3)
...
if dim_variance_history['core']:
    ...
    axs[1,0].axhline(0.1, color='r', ls='--', lw=2)
...
if dim_variance_history['detail']:
    ...
    axs[1,1].axhline(0.1, color='r', ls='--', lw=2)
